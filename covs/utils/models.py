...
def make_lenet5_big_fn(data_info):
...
def make_lenet5_fn(data_info, activation=jax.nn.relu):
    num_classes = data_info["num_classes"]
...
def get_model(model_name, data_info, **kwargs):
  _MODEL_FNS = {
    ...
    "lenet_swish": functools.partial(
      make_lenet5_fn, activation=jax.nn.swish),
    "lenet_sigmoid": functools.partial(
          make_lenet5_fn, activation=jax.nn.sigmoid),
    "lenet_leaky_relu": functools.partial(
      make_lenet5_fn, activation=jax.nn.leaky_relu),
    "lenet_tanh": functools.partial(
      make_lenet5_fn, activation=jnp.tanh),
