...
def get_task_specific_fns(task, data_info):
  if task == data_utils.Task.CLASSIFICATION:
    ...
    tabulate_metrics = [
      ...
      "test/ens_accuracy", "test/ens_nll", "test/ens_ece"
    ]
...
def _make_perdevice_likelihood_prior_grad_fns(
  ...
  def likelihood_prior_and_grads_fn(params, net_state, batch):
    ...
    (likelihood, net_state), likelihood_grad = loss_val_grad(
        net_apply, params, net_state, batch, True)
...
def make_get_predictions(activation_fn, num_batches=1, is_training=False):
  ...
  def get_predictions(
    ...
    net_state, predictions = jax.lax.scan(get_batch_predictions, net_state, dataset)
