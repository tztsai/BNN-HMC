...
from covs.utils import optim_utils

from covs.utils import script_utils
...
def train_model():
  ...
  (train_set, test_set, net_apply, params, net_state, key,
   log_likelihood_fn, log_prior_fn, _, predict_fn, ensemble_upd_fn,
   metrics_fns, tabulate_metrics) = script_utils.get_data_model_fns(args)
  ...
  for iteration in range(start_iteration, args.num_epochs):
    ...
    if (iteration % args.eval_freq == 0) or (iteration == args.num_epochs - 1):
      _, test_predictions, train_predictions, test_stats, train_stats_ = (
          script_utils.evaluate(net_apply, params, net_state,
                                train_set, test_set,
