...
def get_mfvi_model_fn(net_fn, params, net_state, seed=0, sigma_init=0.):
  ...
  def sample_parms_fn(params, state):
    ...
    params_sampled = jax.tree_multimap(lambda m, s, n: m + n * s, mean, std,
                                       noise)
    new_mfvi_state = {"net_state": copy.deepcopy(state["net_state"]),
                      "mfvi_key": new_key}
  ...
  def mfvi_apply_mean_fn(params, state, _, batch, is_training):
    ...
    mean = params["mean"]
    predictions, new_net_state = net_fn(mean, state["net_state"],
                                        None, batch, is_training)
    new_mfvi_state = {"net_state": copy.deepcopy(new_net_state),
...
def make_kl_with_gaussian_prior(weight_decay, temperature=1.):
  ...
  Args:
    ...
    temperature: temperature of the posterior, corresponds to the weight of
      the KL term in the ELBO.

  Returns a function that takes the MFVI parameters and returns the KL
  divergence between the posterior and the prior weighted by the temperature.
  """
  def kl_fn(params):
